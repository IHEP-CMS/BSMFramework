...
class eventSelector : public baseTree{
public:
  eventSelector(std::string name, TTree* tree, bool debug, const edm::ParameterSet& cfg, edm::ConsumesCollector && iC);
  ~eventSelector();
  ...
  void FillJetVectors(const edm::Event& iEvent, std::vector<const reco::Candidate*> looseleps, std::vector<const pat::Jet*>& leading_jets, std::vector<const pat::Jet*>& subleading_jets ,std::vector<const pat::Jet*>& leading_btags, std::vector<const pat::Jet*>& subleading_btags, vector<pair<double,int> >& jet_csv_pos);
  //void Clear();
  //bool ApplySelection();

private:
  //eventSelector(){};//Line may be deprecated.
