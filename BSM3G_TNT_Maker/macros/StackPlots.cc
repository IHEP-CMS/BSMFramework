...
//Corrections
const double xSecs[]        = {6025.2,   831.76,   35.6,    35.6,     61526.7,  118.7,   66.1,   15.4};
const double evtsRead[]     = {9002488., 4992231., 995600., 1000000., 7361970., 989608., 996920.,998848.};
const double Luminosity     = 47.879; //pb^-1
...
const int posvtcr = 0;
//Kinematics
const char *variables[]         = {"Muon_pt"    ,"Muon_eta","Tau_pt"      ,"Tau_eta", "bestVertices"};
const char *vartitleXaxis[]     = {"Mu pT (GeV)","Mu eta"  ,"Tau pT (GeV)","Tau eta", "Num Vtx"};
//1=int,2=double,3=vec_int,4=vec_double //2,3 has to be implemented yet
const int    vartype[numvar]    = {   4,   4,  4,   4,  1};
const int    bin[numvar]        = { 100,  25, 25,  25, 50};
const double inRange[numvar]    = {   0,-2.5,  0,-2.5, 0};
const double endRange[numvar]   = {1000, 2.5,500, 2.5, 50};
const int    ylogscale[numvar]  = {   0,   0,  0,   0, 0};
const double setminimum[numvar] = {   0,   0,  0,   0, 0};
const double setmaximum[numvar] = {  50,  10, 50,  10, 100};
 ...
 for(int v=inivar; v<finvar; v++){
  ...
  for(uint r=0; r<rootplas_size; r++){
   ...
   //Put histos in the hstack 
    ...
    if(rootplas[r]=="tW" || rootplas[r]=="tbarW") col = 28;
    h_var->SetFillColor(col+2);
    h_var->SetLineColor(col+2);
    hstack->Add(h_var);
  ...
  //Save plots
  c1->Update();
  string namefile = specsel+variablesv[v]+"_"+selection+".pdf";;
