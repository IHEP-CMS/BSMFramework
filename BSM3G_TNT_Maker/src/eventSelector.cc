...
#include "BSMFramework/BSM3G_TNT_Maker/interface/eventSelector.h"
#include <iostream>
#include <fstream>
#include <ios>
using namespace edm;
...
eventSelector::eventSelector(string treeName, TTree* tree, bool debug, const pset& iConfig, edm::ConsumesCollector && ic):
...
void eventSelector::JECInitialisation(){
...
void eventSelector::SetBranches(){
...
bool eventSelector::isGoodVertex(const reco::Vertex& vtx){
...
double eventSelector::rel_iso_dbc_mu(const pat::Muon& lepton){
...
double eventSelector::rel_iso_dbc_ele(const pat::Electron& el, double rhopog){
...
double eventSelector::get_effarea(double eta){
  double effarea = -1;
...
bool eventSelector::is_loose_muon(const pat::Muon& mu, const reco::Vertex& vtx){
...
bool eventSelector::is_tight_muon(const pat::Muon& mu, const reco::Vertex& vtx){
  bool isloosemu = false;
  if(mu.pt()>26 && TMath::Abs(mu.eta()) < 2.1 && mu.isTightMuon(vtx) && rel_iso_dbc_mu(mu) < 0.15) isloosemu = true;
  return isloosemu;
}

bool eventSelector::is_loose_electron(const pat::Electron& ele, double rhopog){
  bool isele = false;
...
bool eventSelector::is_tight_electron(const pat::Electron& ele, double rhopog){
  bool isele = false;
...
bool eventSelector::is_good_jet(const pat::Jet &j,double rho, double rhoJER, int vtxsize, double channel_jpt_cut){
  bool isgoodjet = true;
...
void eventSelector::GetJER(pat::Jet jet, float JesSF, float rhoJER, bool AK4PFchs, float &JERScaleFactor, float &JERScaleFactorUP, float &JERScaleFactorDOWN){
...
void eventSelector::FillJetVectors(const edm::Event& iEvent, std::vector<const reco::Candidate*> looseleps, std::vector<const pat::Jet*>& leading_jets, std::vector<const pat::Jet*>& subleading_jets, std::vector<const pat::Jet*>& leading_btags, std::vector<const pat::Jet*>& subleading_btags, vector<pair<double,int> >& jet_csv_pos){
...
void eventSelector::FillLeptonVectors(const edm::Event& iEvent, std::vector<const reco::Candidate*>& looseleps, std::vector<const reco::Candidate*>& loose_electrons, std::vector<const reco::Candidate*>& loose_muons, std::vector<const reco::Candidate*>& tightleps, std::vector<const reco::Candidate*>& tight_electrons, std::vector<const reco::Candidate*>& tight_muons){
  edm::Handle<reco::VertexCollection> vtx_h;
  iEvent.getByToken(vtx_h_, vtx_h);
  edm::Handle<double> rhopogHandle;
  iEvent.getByToken(rhopogHandle_,rhopogHandle);
  ...
  //=== Electrons ===
  //is_XXXX_electron selections have no trigger requirements atm.
...
int eventSelector::dilepton_channel(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  FillJetVectors(iEvent, looseleps, leading_jets, subleading_jets, leading_btags, subleading_btags, jet_csv_pos);
  ...
  if(debug_) std::cout << ("eventSelector") << ": DL selection" << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": tightleps.size(): " << tightleps.size() << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": looseleps.size() : " << looseleps.size() << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": jet_num: " << jet_num << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": jetb_num : " << jetb_num << std::endl;
...
int eventSelector::singlelepton_channel(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if(debug_) std::cout << ("eventSelector") << ": SL selection" << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": tightleps.size(): " << tightleps.size() << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": looseleps.size() : " << looseleps.size() << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": jet_num: " << jet_num << std::endl;
  if(debug_) std::cout << ("eventSelector") << ": jetb_num : " << jetb_num << std::endl;
...
void eventSelector::Fill(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  if(debug_) std::cout << ("eventSelector:Fill") << " Running Fill method"<<endl;
